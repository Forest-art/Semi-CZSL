...
class Evaluator:
    ...
    def __init__(self, dset, model):
        ...
        elif dset.phase == 'mask':
            print('Evaluating with mask pairs')
            test_pair_set = set(dset.mask_train_pairs + dset.train_pairs)
            test_pair_gt = set(dset.mask_train_pairs)
        elif dset.phase == 'val':
...
def predict_logits(model, dataset, config, mask=False):
    ...
    with torch.no_grad():
        for idx, data in tqdm(
            ...
            logits = logits.cpu()
            _, pred_pair = logits.type(torch.float).topk(k=1, dim=1)
            all_logits = torch.cat([all_logits, logits], dim=0)
            all_scores = torch.cat([all_scores, scores], dim=0)
            all_attr_gt.append(attr_truth)
            all_obj_gt.append(obj_truth)
            # print(pair_truth, pred_pair)
    ...
    return all_logits, all_scores, all_attr_gt, all_obj_gt, all_pair_gt, loss / len(dataloader)
...
if __name__ == "__main__":
    ...
    if config.open_world and config.threshold is None:
        ...
        evaluator = Evaluator(val_dataset, model=None)
        feasibility_path = os.path.join(
            DIR_PATH, f'dataset/feasibility_{config.dataset}.pt')
        unseen_scores = torch.load(
            feasibility_path,
            map_location='cpu')['feasibility']
        with torch.no_grad():
